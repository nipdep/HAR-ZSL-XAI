...
class SkeletonPickleReader(SkeletonFile):
...
class FileDictIterator:
    def __init__(self, files: dict, keys: List[str] = None, load_original=False, orig_file_loc=None):
        self.files = files
        if not keys:
        ...
        self.orig_file_loc = orig_file_loc

        if load_original:
            assert orig_file_loc
    ...
    def __next__(self):
        if self.ind < len(self.keys):
            x = self.files[self.keys[self.ind]]
            self.ind += 1
            if self.load_original:
                orig = NTU120SkeletonFile(
                    os.path.join(self.orig_file_loc, f"{self.keys[self.ind]}.skeleton")).load_data()
                return x, orig
            else:
                return x
        else:
...
if __name__ == "__main__":
    ...
    pkl_reader = SkeletonPickleReader(file_path="D:\\FYP\\HAR-ZSL-XAI\\data\\ntu120_3danno.pkl")

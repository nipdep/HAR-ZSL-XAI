...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch, class_names=None):
    """Run an evaluation on the validation set while logging metrics, and handle outcome"""

    logger.info("Evaluating on validation set ...")
    eval_start_time = time.time()
    with torch.no_grad():
        aggr_metrics, per_batch = val_evaluator.evaluate(epoch, keep_all=True, class_names=class_names)
    eval_runtime = time.time() - eval_start_time
...
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True, class_names=None):

        # self.model = self.model.eval() #@nipdep
...
class SupervisedRunner(BaseRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[3], torch.nn.CrossEntropyLoss):
            ...
            self.analyzer = analysis.Analyzer(print_conf_mat=True, plot=False)
        else:
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            loss = batch_loss = mean_loss = self.loss_module(predictions, targets)
            ...
            # torch.nn.utils.clip_grad_value_(self.model.parameters(), clip_value=1.0)
            # torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=4.0) # gradient clipping
            self.optimizer.step()
    ...
    def evaluate(self, epoch_num=None, keep_all=True, class_names=None):
        ...
        if self.classification:
            ...
            probs = torch.softmax(predictions, dim=-1)  # (total_samples, num_classes) est. prob. for each class and sample

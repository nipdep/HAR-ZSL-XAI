...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        self.label = matcontent['labels'].astype(int).squeeze() - 1   #(11788,) 
        self.image_files = matcontent['image_files'].squeeze()  
        matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + "_splits.mat")
        # numpy array index starts from 0, matlab starts from 1
        # print("matcontent.keys:", matcontent.keys())
        self.trainval_loc = matcontent['trainval_loc'].squeeze() - 1     #(7057,)
        if opt.dataset == 'CUB':
            self.train_loc = matcontent['train_loc'].squeeze() - 1    #(5875,)
            self.val_unseen_loc = matcontent['val_loc'].squeeze() - 1    #(2946,)
        ...
        self.test_seen_loc = matcontent['test_seen_loc'].squeeze() - 1  #(1764,)
        self.test_unseen_loc = matcontent['test_unseen_loc'].squeeze() - 1    #(2967,)
        self.allclasses_name = matcontent['allclasses_names']    #(200,1)
        self.attribute = torch.from_numpy(matcontent['att'].T).float()   #(200,312) 
        self.attribute_binary = torch.from_numpy(matcontent['binary_att'].T).float()
        self.attri_name = matcontent['attri_name'] #(312,) 
        ...
        self.ntrain = self.train_feature.size()[0]      #7057
        self.ntest_unseen = self.test_unseen_feature.size()[0]  #2967
        self.ntest_seen = self.test_seen_feature.size()[0]  #1764
        self.ntrain_class = self.seenclasses.size(0)    #150
        self.ntest_class = self.unseenclasses.size(0)   #50
        self.train_class = self.seenclasses.clone() #150种类别的id
        self.allclasses = torch.arange(0, self.ntrain_class + self.ntest_class).long()
        self.train_mapped_label = map_label(self.train_label, self.seenclasses) #对于可见类，每一个id对应哪一个类别 （7057）
...
def default_flist_reader(opt, image_files, img_loc, image_labels, dataset):
    ...
    if opt.dataset == "AWA2" or opt.dataset == "SUN" or opt.dataset == 'CUB':
        for image_file, image_label,img_lo in zip(image_files, image_labels,img_loc):
            ...
            text_matrix = opt.attribute_binary[image_label]
